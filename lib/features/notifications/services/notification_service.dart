...
class NotificationService {
  ...
  Future<void> init(Settings settings) async {
    ...
    _notificationCacheManager = NotificationCacheManager(
  ...
  Future<void> _initHive() async {
    ...
    if (!Hive.isAdapterRegistered(28)) {
      // Corrected typeId for NotificationModelAdapter
      Hive.registerAdapter(
        NotificationModelAdapter(),
      ); // Using the imported adapter
    }

    if (!Hive.isAdapterRegistered(29)) {
      // Corrected typeId for NotificationTypeAdapter
      Hive.registerAdapter(
  ...
  Future<void> _initLocalNotifications() async {
    ...
    final WindowsInitializationSettings windowsSettings =
        WindowsInitializationSettings(
          appName: 'Wanzo',
          appUserModelId: 'com.wanzo.app',
          guid: 'd49b0314-ee7a-4e37-b5f1-0c7c45a0b64a',
        );
  ...
  Future<void> _showLocalNotification(NotificationModel notification) async {
    ...
    const AndroidNotificationDetails androidDetails =
        AndroidNotificationDetails(
          ...
          'Notifications Wanzo',
          channelDescription:
              'Canal pour les notifications de l\'application Wanzo',
  ...
  Future<void> _loadCachedNotifications() async {
    try {
      final cachedNotifications =
          await _notificationCacheManager.getCachedNotifications();
      ...
      debugPrint(
        '${cachedNotifications.length} notifications chargées depuis le cache',
      );
    } catch (e) {
      Logger.error(
        'Erreur lors du chargement des notifications en cache',
        error: e,
      );
    }
  }
  ...
  Future<void> addNotificationWithOfflineSupport(
    ...
    try {
      ...
      Logger.info(
        'Notification ajoutée avec support hors ligne: ${notification.id}',
      );
    } catch (e) {
      Logger.error(
        'Erreur lors de l\'ajout de notification avec support hors ligne',
        error: e,
      );
    }
  }
  ...
  Future<int> getUnsyncedNotificationsCount() async {
    try {
      ...
      return sqflite.Sqflite.firstIntValue(result) ??
          0; // Using the imported Sqflite with namespace
    } catch (e) {
      Logger.error(
        'Erreur lors du comptage des notifications non synchronisées',
        error: e,
      );
      return 0;
    }
  ...
  void _onConnectivityChanged() {
    ...
    Logger.info(
      'Connectivité changée: ${isConnected ? 'En ligne' : 'Hors ligne'}',
    );
  ...
  Future<void> _syncOfflineNotifications() async {
    try {
      ...
      Logger.info(
        'Démarrage de la synchronisation des notifications hors ligne',
      );
      ...
      for (final map in unsyncedMaps) {
        ...
        try {
          ...
          Logger.error(
            'Erreur lors de la synchronisation de la notification ${notification.id}',
            error: e,
          );
        }
      }
